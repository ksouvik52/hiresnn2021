...
def validate_fgsm_bb(val_loader, model_bb, model, args, eps=0.031):
    if args.dataset == 'CIFAR10':
        mean = torch.Tensor(np.array([0.4914, 0.4822, 0.4465])[:, np.newaxis, np.newaxis])
        mean = mean.expand(3, 32, 32).cuda()
        std = torch.Tensor(np.array([0.2023, 0.1994, 0.2010])[:, np.newaxis, np.newaxis])
    ...
    for i, (data, target) in enumerate(val_loader):
        ...
        #Here generate the perturbed data from model_bb
        perturbed_data = attacker.attack_method(model_bb, data, target, args)
        perturbed_data.sub_(mean).div_(std)
        # Use the model_bb generated perturbed data to evaluate on model
    ...
    print('Top1 FGSM:{}'.format(top1_fgsm))
    return top1_fgsm


def validate_pgd_bb(val_loader, model_bb, model, eps=0.031, K=7, a=0.01):
    if args.dataset == 'CIFAR10':
        mean = torch.Tensor(np.array([0.4914, 0.4822, 0.4465])[:, np.newaxis, np.newaxis])
        mean = mean.expand(3, 32, 32).cuda()
        std = torch.Tensor(np.array([0.2023, 0.1994, 0.2010])[:, np.newaxis, np.newaxis])
    ...
    for i, (data, target) in enumerate(val_loader):
        ...
        for _ in range(K):
            ...
            in1 = invar - mean
            in1.div_(std)
            #Here generate the output for grad computation from model_bb
            output,_ = model_bb(in1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_snn_bb',      default='',                 type=str,       help='pretrained SNN for BB inference')
    ...
    pretrained_snn_bb   = args.pretrained_snn_bb
    epochs              = args.epochs
    ...
    if architecture[0:3].lower() == 'vgg':
        ...
        model_bb = VGG_SNN(vgg_name = architecture, activation = activation, labels=labels, timesteps=timesteps, leak=leak, default_threshold=default_threshold, dropout=0.2, kernel_size=kernel_size, dataset=dataset)
  
    if pretrained_snn:
        if pretrained_snn_bb:
            state_bb = torch.load(pretrained_snn_bb, map_location='cpu')
            missing_keys, unexpected_keys = model_bb.load_state_dict(state_bb, strict=False)
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        top1_fgsm = validate_fgsm_bb(test_loader, model_bb, model, args, eps=0.031)
        top1_pgd = validate_pgd_bb(test_loader, model_bb, model, eps=0.031, K=7, a=0.01)
